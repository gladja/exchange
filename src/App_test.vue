<template>
  <div id="app" class="main">
    ...
    <div class="container">
      <div class="field-input">
        <div class="field-input__wrapper">
          ...
          <div>
            <div
                v-if="maxSelect < Number(10000)"
                class="field-input__second"
            >
              <div>
                <input
                    @input="onChange"
    ...
    <div
        class="field-input__alert"
        v-if="maxSelect > Number(10000)"
    >
      <p>Максимальная сумма желаемой валюты может быть не более <strong>10 000$ (в USD)</strong></p>
    </div>
    <div class="field-input__alert" v-else>{{maxSelect}} $</div>
...
export default {
  ...
  computed: {
    maxSelect() {
      return this.coins.USD * this.value;
    }
  },
  methods: {
    init() {
      setInterval(this.onChange, 5000);
      setInterval(this.onChange2, 5000);
    },
    ...
    async onChange2() {
      ...
      const result = await Promise.all(coins.map(str => this.getValue2(str === 'result' ? undefined : str)))
    ...
    async getValue2(selectedTo = this.selected2) {
      const API_KEY = '1d17986cc6dc17aa021eb8624bb5bb3e95309659ddb75652711f4baf55d7abbb';
      const loadTicker = () =>
          fetch(
              `https://min-api.cryptocompare.com/data/price?fsym=${this.selected}&tsyms=${selectedTo}&api_key=${API_KEY}`
